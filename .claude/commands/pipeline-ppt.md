...
### 第三步：确定 Python 路径（强制，不可跳过）

**按以下顺序查找 Python 路径，取第一个成功的：**

1. 读取**当前工作区根目录**的 `.claude_settings.json`，检查 `python_path` 字段
2. 读取 `SKILL_DIR/.claude_settings.json`，检查 `python_path` 字段

- **如果找到且 `python_path` 非空** → 记为 PYTHON，后续所有 `python` 命令替换为该完整路径
- **如果都未找到** → **必须用 AskUserQuestion 询问用户（不可跳过、不可猜测）**：
  ```
...
**后续所有步骤中出现的 `python` 命令，一律替换为 PYTHON 的完整路径。**
...
- 解析成功后，从 `_selected_wiki_docs.json` 读取选中路径，再用 `read_file` 完整读取每个选中文件内容，作为第七步素材
...
**处理逻辑：**
- **选 1-8** → 从 `SKILL_DIR/demo/ppt_live/templates.json` 读取对应 theme 色值 → 进入 **HTML 模式生成**（第七步）
- **选 9（已上传模板）** → 进入 **克隆模式生成**（第七步）
