...
**处理逻辑：**
- 从 `SKILL_DIR/ppt_live/templates.json` 读取对应 theme 色值 → 进入第五步生成

---

### 第五步：生成 PPT（三阶段流程）

经过「内容分析 → 大纲规划 → 逐页生成」三阶段。
...
5. **不写临时 Python 脚本** — 只用 `create_file` 写 JSON + CLI 推送
6. **绝不嵌入 HTML 到 shell** — 先写 JSON 文件，再 push 文件路径
