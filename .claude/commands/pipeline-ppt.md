...
### 第一步：获取 Python 路径 + 一键初始化

#### 1a. 获取 Python 路径（必须先做）

用 Read 工具读取工作区根目录的 `.claude_settings.json`，取其中的 `python_path` 字段。

- **如果文件存在且包含 `python_path`** → 记为 `PYTHON`（例如 `C:\Users\86187\AppData\Local\Programs\Python\Python312\python.exe`）
- **如果文件不存在或缺少 `python_path`** → 提示用户：
  ```
  ⚠️ 未找到 Python 路径配置。请先运行 /pipeline-config 配置 Python 路径，
  或在 .claude_settings.json 中添加 "python_path": "你的python.exe完整路径"
  ```
  然后**立即停止**。

**后续所有 `python` 命令一律替换为 `PYTHON` 的完整路径**。禁止使用裸 `python` 或 `python3`。
...
> 将 `PYTHON` 替换为 1a 获取的完整路径，例如：
> `"C:\Users\86187\AppData\Local\Programs\Python\Python312\python.exe" tools/pipeline_ppt_fastpath.py bootstrap --port 5679`

该命令由 Python 脚本确定性完成以下全部流水线工作（< 1 秒）：
1. ✅ 自动定位 SKILL_DIR（搜索 `~/.agents/skills/` 和 `~/.claude/skills/`）
2. ✅ 校验 `.llm_config.json` LLM 配置是否可用
3. ✅ 启动或复用预览服务器（`http://localhost:5679`）
4. ✅ 扫描 wiki 目录，按分类编号打印所有 `.md` 文件
5. ✅ 生成 `output/_wiki_index_map.json` 编号映射文件
...
⚠️ **绝对禁止**以下行为（历史上造成 20 分钟级超时的根因）：
- 拆分初始化为多条 shell 命令
- 使用 bash/PowerShell 变量赋值探测路径
- 手写 if/elif/fi 或 foreach 检测目录
- 直接使用裸 `python` 或 `python3`（Windows bash 环境中不在 PATH）
- 上述全部工作已封装在 Python 脚本中，一条命令即可完成

### 第二步：交互式三步选择（严格顺序，逐步提问，全程中文）
...
```bash
"PYTHON" tools/pipeline_ppt_fastpath.py parse-selection "用户输入原文"
```
...
- 解析成功后，从 `_selected_wiki_docs.json` 读取选中路径，再用 `read_file` 完整读取每个选中文件内容，作为第三步素材
...
**处理逻辑：**
- **选 1-8** → 从 `SKILL_DIR/demo/ppt_live/templates.json` 读取对应 theme 色值 → 进入 **HTML 模式生成**（第三步）
- **选 9（已上传模板）** → 进入 **克隆模式生成**（第三步）
...
2. **一条命令初始化** — `"PYTHON" tools/pipeline_ppt_fastpath.py bootstrap` 完成路径探测 + 配置校验 + 服务启动 + wiki 编号，禁止拆分为多条 shell 命令
3. **禁止裸 `python`** — 所有 Python 调用必须使用 `.claude_settings.json` 中的完整路径
