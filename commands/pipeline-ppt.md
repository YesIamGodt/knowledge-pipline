...
**正确做法 — 两步：**
1. 用 `create_file` 为每页创建独立 HTML 文件到 `SKILL_DIR/output/_tmp_slides/` 目录：
   - `SKILL_DIR/output/_tmp_slides/slide-01.html` — 封面页的完整 HTML
   - `SKILL_DIR/output/_tmp_slides/slide-02.html` — 第二页的完整 HTML
   - `SKILL_DIR/output/_tmp_slides/slide-03.html` — 以此类推...
...
2. 终端执行推送（一条命令推送整个目录）：
   ```
   python "SKILL_DIR/demo/ppt_live/pptx_live.py" push "SKILL_DIR/output/_tmp_slides/"
   ```
