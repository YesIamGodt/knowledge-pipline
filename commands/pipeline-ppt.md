...
第 ③ 步必须这样做：
1. **先执行稳定命令获取模板列表，不要自己读两个 JSON 文件再拼接：**
  ```
  python "SKILL_DIR/demo/ppt_live/pptx_live.py" templates-json
  ```
2. 从该命令输出中读取：
  - `templates`：当前所有可用模板
  - `count`：当前模板总数
  - `upload_option_index`：最后那个“上传新 PPTX 模板”的编号
3. 按命令输出结果，**一次性完整列出当前所有可用模板**，按返回的 `index` 展示
...
**⚠️ 不要再让 agent 自己拼模板列表。** 模板列表必须以 `templates-json` 命令输出为准，这样内置模板、用户模板、编号顺序、上传入口编号都只有一个事实来源。
...
- **选中某个用户上传模板** → 把它当成一个普通主题模板使用，直接读取其持久化的 `theme + fonts` → 进入 **HTML 模式生成**（第五步）
...
**核心原则：上传 PPT 模板的输入是 PPT，输出只是一个新的可选风格。**

也就是说：
- 原本有 8 个内置风格
- 用户上传并识别成功后，就变成 9 个可选风格
- 抽取粒度只需要和现有 `theme` 结构一致：背景色、标题色、正文色、强调色、弱化色、徽标色、徽标背景、字体等
- **不要**进入 clone、outline、按原 PPT 布局复刻之类的流程
...
2. 终端执行风格抽取并持久化：
  ...
  ```
3. 读取识别结果，向用户展示抽取到的主题信息：模板名称、背景色、标题色、正文色、强调色、字体
4. **必须同时展示：当前模板总数 + 新模板编号**
   - 例如：`当前可用模板总数: 9`
   - 例如：`新模板编号: 9`
   - 明确告诉用户：`现在可直接输入编号 9 选择这个新风格`
5. **必须再次完整列出全部模板，并把新模板标记为「刚上传」**
  - 例如：`9. 📎 浅色版模板 [用户上传] [刚上传]`
  - 让用户能在完整列表里一眼定位新增模板
6. 识别成功后，**留在第 ③ 步**，立即重新执行：
  ```
  python "SKILL_DIR/demo/ppt_live/pptx_live.py" templates-json
  ```
7. 按最新命令输出重新展示完整模板列表；此时新上传模板已经是一个普通可选风格，用户直接输入它的编号即可

**⚠️ 严禁的错误流程：**
- 上传模板后生成 `_tmp_outline.json`
- 上传模板后执行 `clone-generate`
- 把上传模板当成“按原 PPT 版式克隆”的入口
- 因为这里要的只是**风格抽取**，不是版式克隆

---

### 第五步：生成 PPT（三阶段流程）

无论选择内置主题还是用户上传主题，都统一经过「内容分析 → 大纲规划 → HTML 逐页生成」三阶段。
...
**⚠️ 无论主题来自哪里，生成方式都相同：**
- 内置主题：直接使用 `templates.json` 中的 `theme + fonts`
- 用户上传主题：直接使用 `user_templates.json` 中抽取并持久化的 `theme + fonts`
- 二者都只是“风格来源不同”，**生成方式完全一致**
- **禁止**因为选了用户上传主题而切换到 clone / outline / 复刻原 PPT 布局
...
**⚠️ 不存在“用户上传模板 = 克隆模式”这条分支。**

上传模板只会新增一个可选风格；真正生成 PPT 时，仍然使用上面的 HTML 模式逐页生成。
