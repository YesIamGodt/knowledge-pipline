...
```
python "SKILL_DIR/ppt_live/server.py"
```
...
用 `list_dir` 扫描四个目录，用 `read_file` 读取每个 .md 前 5 行获取 `title`：
- `SKILL_DIR/wiki/sources/` → 源文档摘要
- `SKILL_DIR/wiki/entities/` → 实体
- `SKILL_DIR/wiki/concepts/` → 概念
- `SKILL_DIR/wiki/syntheses/` → 综合分析

---

#### 第 ① 步 — 选择 Wiki 文档（必选，禁止跳过）

使用 AskUserQuestion，**必须提供 options 列表，`allowFreeformInput: false`**。

将扫描到的 wiki 文档作为选项列出，支持多选（`multiSelect: true`）：
...
- 用户**必须**从列表中选择至少一篇文档
- **禁止**自由输入、跳过、或输入 0
- 如果 wiki 为空（无任何文档），提示用户先运行 `/pipeline-ingest` 摄入文档，然后停止

选择后，用 `read_file` 读取所选文档的完整内容作为 PPT 素材。
...
**处理逻辑：**
- **选 1-8** → 从 `SKILL_DIR/ppt_live/templates.json` 读取对应 theme 色值 → 进入 **HTML 模式生成**（第五步）
...
2. 终端执行上传识别：
   ```
   python "SKILL_DIR/ppt_live/pptx_live.py" upload-template "用户给的路径"
   ```
...
参考 `SKILL_DIR/ppt_live/SKILL.md` 中的 12 种 HTML 模板，但 LLM 有完全创意自由。
...
2. 终端执行推送：
   ```
   python "SKILL_DIR/ppt_live/pptx_live.py" push "SKILL_DIR/output/_tmp_slides.json"
   ```
...
2. 终端执行克隆生成：
   ```
   python "SKILL_DIR/ppt_live/pptx_live.py" clone-generate "模板路径" "SKILL_DIR/output/_tmp_outline.json" -o "SKILL_DIR/output/文件名.pptx"
   ```
...
用户的每句话翻译为 CLI 命令（所有命令前缀 `python "SKILL_DIR/ppt_live/pptx_live.py"`）：
