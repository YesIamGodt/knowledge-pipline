...
import json
slides = [
    {"layout_index": 0, "placeholders": {"0": "标题", "1": "副标题"}},
    {"layout_index": 1, "placeholders": {"0": "章节标题", "1": "• 要点1\n• 要点2"}},
]
...
## 十四、模板模式 — 原生可编辑 PPTX 生成（推荐）

> **核心优势**：生成的 PPTX 100% 原生可编辑 — 文本框、表格、图表都可直接在 PowerPoint 中修改。

### 两种模式对比

| 维度 | HTML 模式（预览） | 模板模式（推荐） |
|------|-------------------|------------------|
| 生成方式 | LLM 写 HTML → 浏览器截图导出 | LLM 写结构化 JSON → python-pptx 填充 |
| 可编辑性 | 伪可编辑（截图背景 + 文字覆盖） | **原生可编辑**（真实文本框/占位符） |
| 视觉保真 | 高（HTML 自由度大） | 依赖模板质量 |
| 适用场景 | 需要高度定制视觉 | 正式输出、企业模板、需要二次编辑 |

### 工作流

```
用户上传 .pptx 模板 → 解析母版布局/占位符 → LLM 生成结构化 slide JSON → python-pptx 填充占位符 → 原生 .pptx
```
...
✅ 模板已上传: my-template
   文件: my-template.pptx
   布局数: 5
     [0] Title Slide → Title 1(idx=0), Subtitle 2(idx=1)
     [1] Title and Content → Title 1(idx=0), Content Placeholder 2(idx=1)
     [2] Section Header → Title 1(idx=0), Text Placeholder 2(idx=1)
     [3] Two Content → Title 1(idx=0), Content Placeholder 2(idx=1), Content Placeholder 3(idx=2)
     [4] Blank → 无占位符
   主题色: #4472C4, #ED7D31, ...
   字体: Calibri, 微软雅黑
```
...
LLM 根据模板布局信息生成 JSON。每个 slide 指定 `layout_index`（或 `layout_name`）+ 占位符内容：
...
当 slide JSON 使用 `type` 字段（如 `"type": "title"`）而非 `layout_index` 时，系统自动将 slide 类型映射到模板布局：

| Slide type | 匹配的布局名关键词 |
|------------|-------------------|
| title | title, 标题, cover, 封面 |
| bullets | content, 内容, body |
| comparison | two, 两栏, 对比, compare |
| section | section, 章节, header |
| quote | blank |
| metric | blank |
| timeline | content, 内容 |
...
| 方法 | 路径 | 功能 |
|------|------|------|
| POST | `/api/upload-template` | 上传模板 `{path: "..."}` |
| GET | `/api/templates` | 列出所有模板 |
| GET | `/api/template/<id>` | 模板详情 |
| POST | `/api/template/<id>/generate` | 从模板生成 `{slides: [...], path: "..."}` |
| POST | `/api/export` | 增强 — 支持 `template_id` 参数 |
